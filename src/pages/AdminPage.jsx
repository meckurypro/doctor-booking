      ...
      >
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>
            {prompts.length} version{prompts.length !== 1 ? 's' : ''} · Used{' '}
        ...
        {isEditing ? (
          <div className="flex gap-2">
            <Button onClick={() => setIsEditing(false)} variant="ghost" size="sm" icon={X}>
              Cancel
            </Button>
            <Button onClick={handleSave} loading={saving} variant="primary" size="sm" icon={Save}>
              Save
            </Button>
          </div>
        ) : (
          <Button onClick={() => setIsEditing(true)} variant="outline" size="sm" icon={Edit3}>
            Edit
          </Button>
        )}
      </div>
...
export default function AdminPage() {
  ...
  // Uses a dedicated reject RPC to avoid double-calling approve
